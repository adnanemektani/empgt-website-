...
import { Plug, Zap, Factory, Leaf, ArrowRight } from "lucide-react";
import { useTranslations } from "next-intl";
import { Link } from "@/i18n/navigation";

const IRVE_ICONS = [Plug, Zap, Factory, Leaf];

export default function Irve({ showHeader = true }: { showHeader?: boolean }) {
  const t = useTranslations("Irve");
  const items = t.raw("items") as { title: string; text: string }[];

  return (
    <section id="irve" className="relative overflow-hidden py-24 lg:py-32">
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className={`${showHeader ? "mt-14" : "mt-2"} grid gap-6 sm:grid-cols-2 lg:grid-cols-4`}>
          {items.map((item, i) => {
            const Icon = IRVE_ICONS[i];
            return (
              <motion.div
                key={item.title}
